...
npm install @macrix-technology-group/bpmn-forge@0.3.5

# from GitHub (pin to a release tag)
npm install "git+https://github.com/Macrix-Technology-Group/bpmn-forge.git#0.3.5"
```
...
When pinning from GitHub, pin to a tag (`#0.3.5`) — never to `main` — so a future commit on `main` doesn't silently change your dependency.
...
`#0.3.5` is the git ref to check out. Released tags are listed at <https://github.com/Macrix-Technology-Group/bpmn-forge/tags>.
...
### Migrating to 0.3.5 (extended trunk staggering)

If you're upgrading from 0.3.4:

- **Trunk-stagger pass now also covers first/last segments**, not just middles. Two routed edges sharing a horizontal or vertical track that involves anchor endpoints (e.g. Tier 2's first segment running at y=Y_DA toward Parallel Investigation, while Pull Back's last segment ending at Diagnostic Abandoned at the same y) are now staggered so each renders on its own track. Previously only middle segments were considered, so first/last collisions slipped through.
- **Event-anchored segments are pinned at depth 0.** Events are circles; shifting an anchor along the face would take it off the visible perimeter. When a conflict involves an event-anchored segment, the OTHER segment moves; the event-anchored one stays put.
...
// package.json
{
  "dependencies": {
    "@macrix-technology-group/bpmn-forge":
      "git+https://github.com/Macrix-Technology-Group/bpmn-forge.git#0.3.5"
  }
}
```
...
```bash
# yarn
yarn add "@macrix-technology-group/bpmn-forge@git+https://github.com/Macrix-Technology-Group/bpmn-forge.git#0.3.5"

# pnpm
pnpm add "github:Macrix-Technology-Group/bpmn-forge#0.3.5"
```
...
```bash
npm install "git+ssh://git@github.com/Macrix-Technology-Group/bpmn-forge.git#0.3.5"
```
...
| `npm ERR! 404` on git URL | Check the spelling of the org/repo name and the tag (`#0.3.5`). Tags are at <https://github.com/Macrix-Technology-Group/bpmn-forge/tags>. |
